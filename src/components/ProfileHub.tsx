    ...
    return shell(
      <>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Sign in with Google, or just your name and a private password kept on this device. Perfect for kids, students, grown-ups and grandparents.
        </p>
        <button
          type="button"
          onClick={() => void signInWithGoogle()}
          disabled={googleBusy}
          className="mt-4 flex w-full items-center justify-center gap-3 rounded-full border border-border bg-card px-6 py-3 text-sm font-bold shadow-[var(--shadow-soft)] transition hover:bg-muted disabled:opacity-60"
        >
        ...
        {googleError && <p className="mt-2 text-sm font-semibold text-destructive">{googleError}</p>}
        <div className="mt-4 flex items-center gap-3 text-xs font-semibold text-muted-foreground" aria-hidden="true">
          <span className="h-px flex-1 bg-border" /> or fill in by hand <span className="h-px flex-1 bg-border" />
        </div>
        {googleName && (
